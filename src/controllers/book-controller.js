...
const uploadBook = async (req, res) => {
  ...
  try {
    ...
    const archivoPDF = req.files["archivoPDF"]
      ? req.files["archivoPDF"][0].buffer
      : null;
    const archivoAudio = req.files["archivoAudio"]
      ? req.files["archivoAudio"][0].buffer
      : null;
    const archivoPortada = req.files["archivoPortada"]
      ? req.files["archivoPortada"][0].buffer
      : null;
    ...
    res
      .status(500)
      .json({ error: "Error cheee al subir los archivos del libro" });
  }
};
